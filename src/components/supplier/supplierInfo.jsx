...
import EditCompanyModal from '../producer/EditCompanyModal'
import CompleteModal from './CompleteModal'
import CancelModal from './CancelModal'
import ConfirmModal from './ConfirmModal'

export function SupplierData({companyData}) {
...
export function SupplierRow2({freightTakenParam}) {
  const [successInfoModalOpen, setSuccessInfoModalOpen] = useState(false);
  ...
  const completeModalHandleClick = (data) => {
    setCompleteModalOpen(true);
    setCompleteModalData(data);
  }
  const completeModalHandleClose = () =>{
    setCompleteModalOpen(false);
  } 
  const cancelModalHandleClick = (data) => {
    ...
    <>
    {
      completeModalOpen ?
        <CompleteModal
          ...
          handleClose = {completeModalHandleClose}
    ...
    }
    {
      cancelModalOpen ?
        <CancelModal
          ...
          handleClose = {cancelModalHandleClose}
          onSuccess = {()=> setSuccessInfoModalOpen(true)}
    ...
    }
    {
        successInfoModalOpen ?
          <SuccessInfoModal
            ...
            message = "You have completed transport!" /> 
          : ''
      }
    <React.Fragment>
      <div style={{marginLeft:10, marginRight:10}}>
                  ...
                  <TableRow>
                    ...
                    <TableCell align="center">
                        <Button
                          ...
                          onClick={() => completeModalHandleClick(freightTakenn)}
                          >
                            Complete
                        </Button>
                        <Button
                          ...
                          onClick={()=> cancelModalHandleClick(freightTakenn) }
                          >
                            Cancel freight
                        </Button>
                    </TableCell>
...
export function SupplierRow({freightFreeParam}) {
    const [freightFree] = useState(freightFreeParam);
    const [successInfoModalOpen, setSuccessInfoModalOpen] = useState(false);
    const [confirmModalOpen, setConfirmModalOpen] = useState('');
    const [confirmModalData, setConfirmModalData] = useState();

    const confirmModalHandleClick = (data) => {
      setConfirmModalOpen(true);
      setConfirmModalData(data);
    }
    const confirmModalHandleClose = () =>{
      ...
      <>
        {
          successInfoModalOpen ?
            <SuccessInfoModal
              ...
              message="Freight has been successfully taken." />
            : ''
        }
        {
          confirmModalOpen ?
                  ...
                  <TableBody>
                    {freightFree ? freightFree.map((freightFreee, i) => (
                      <TableRow key={i}>
                        ...
                        <TableCell key={freightFreee.companyName} align="center">
                          {freightFreee.companyName}
                        </TableCell>
                        <TableCell key={freightFreee.email} align="center">
                          {freightFreee.email}
                        </TableCell>
                        <TableCell key={freightFreee.VAT} align="center">
                          {freightFreee.VAT}
                        </TableCell>
                        <TableCell align="center">
                          <Button
                            ...
                            onClick={() => confirmModalHandleClick(freightFreee.idFreight)}
                          >
                            Take Freight
                          </Button>
                        </TableCell>
  ...
  export function SupplierRow3({freightCompletedParam}) {
    debugger
